...
import {
  MessageCircle, Send, Bot, User, Sparkles, Brain, Loader2, Square, Plus, Menu
} from 'lucide-react';
...
/* Tailwind safelist – never purge these colour classes
  text-red-500 border-red-500 bg-red-500/10
  text-amber-500 border-amber-500 bg-amber-500/10
  text-green-500 border-green-500 bg-green-500/10
*/

const STREAM_TIMEOUT = 15_000; // 15 s safety timer
...
export function AIChat() {
  ...
  // Initialise session and load messages from Supabase
  useEffect(() => {
    (async () => {
      try {
        const { user } = await auth.getCurrentUser();
        if (!user) { console.warn('AIChat: user not logged in; messages will not persist.'); return; }
        ...
        const normalized = (sessRows || []).map((s: any) => ({ id: s.id, title: s.title || 'Untitled Chat', updated_at: s.updated_at }));
        ...
        if (!sid) {
          const { data: created, error: cErr } = await supabase
            ...
            .single();
          if (cErr) { console.error('AIChat: error creating session', cErr); return; }
          sid = created.id;
        ...
        if (mErr) { console.error('AIChat: error fetching messages', mErr); return; }
      ...
      } catch (e) { console.error('AIChat init error:', e); }
    })();
  }, []);
  ...
  const handleSendMessage = async () => {
    ...
    if (!sessionId) { console.warn('AIChat: No session yet; try again after session initializes.'); return; }

    const questionText = input.trim();
    const userMessage: ChatMessage = { id: Date.now().toString(), type: 'user', content: questionText, timestamp: new Date() };
    ...
    setMessages(prev => [...prev, { id: aiTempId, type: 'ai', content: '', timestamp: new Date(), confidence: undefined }]);
    ...
    try {
      ...
      setMessages(prev => prev.map(m => (m.id === aiTempId ? fallback : m)));
    } finally {
      clearTimeout(timer);
      setIsStreaming(false);
      setLoading(false);
            ...
            <div className="lg:hidden">
              <Sheet>
                <SheetTrigger asChild>
                  <Button variant="outline" size="sm" className="text-xs px-3 py-2"><Menu className="w-3 h-3 mr-1" /> Sessions</Button>
                </SheetTrigger>
                <SheetContent side="left" className="w-[88%] sm:w-[360px]">
                  <SheetHeader><SheetTitle>Your chats</SheetTitle></SheetHeader>
                  <div className="mt-4 space-y-1">
                    {sessions.map(s => (
                      <button key={s.id} onClick={async () => { if (s.id === sessionId) return; setSessionId(s.id); const { data: rows, error: mErr } = await supabase.from('ai_chat_messages').select('id, role, content, confidence, created_at').eq('session_id', s.id).order('created_at', { ascending: true }); if (!mErr) setMessages((rows || []).map(r => ({ id: String(r.id), type: r.role as 'user' | 'ai', content: r.content, timestamp: new Date(r.created_at), confidence: r.confidence ?? undefined }))); }} className={`w-full text-left p-3 rounded-md border ${s.id === sessionId ? 'border-crypto-green/50 bg-crypto-green/10' : 'border-transparent hover:bg-muted/30'}`}>
                    ...
                    {sessions.length === 0 && <div className="text-xs text-muted-foreground">No chats yet</div>}
                  </div>
                </SheetContent>
              </Sheet>
            </div>
              ...
              <Collapsible open={sidebarOpen} onOpenChange={setSidebarOpen}>
                <div className="flex items-center justify-between px-3 pt-3 pb-2">
                  ...
                  <CollapsibleTrigger asChild>
                    <Button variant="outline" size="sm" className="h-7 px-2 text-xs"><Menu className="w-3 h-3 mr-1" /> Sessions</Button>
                  </CollapsibleTrigger>
                </div>
                <CollapsibleContent className={`overflow-y-auto px-2 pb-2 ${sidebarOpen ? '' : 'hidden'}`}>
                  <div className="space-y-1">
                    ...
                    {sessions.length === 0 && <div className="text-xs text-muted-foreground px-1">No chats yet</div>}
                  </div>
                </CollapsibleContent>
              </Collapsible>
            </div>
          ...
          <div className="flex-1 flex flex-col min-w-0">
            <div className="flex-1 overflow-y-auto space-y-4 pr-2">
              {messages.map(message => (
                <div key={message.id} className={`flex ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] sm:max-w-[80%] rounded-lg p-3 break-words overflow-wrap-anywhere ${message.type === 'user' ? 'bg-crypto-green/20 text-foreground ml-2 sm:ml-4' : 'bg-muted/50 text-foreground mr-2 sm:mr-4'}`}>
                    <div className="flex items-start space-x-2">
                      {message.type === 'ai' && <Bot className="w-4 h-4 text-crypto-green mt-1 flex-shrink-0" />}
                      {message.type === 'user' && <User className="w-4 h-4 text-crypto-blue mt-1 flex-shrink-0" />}
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 gap-2">
                  {suggestedQuestions.map((q, i) => (
                    <Button key={i} variant="outline" size="sm" onClick={() => handleSuggestedQuestion(q)} className="w-full justify-start text-xs hover:bg-crypto-green/10 hover:border-crypto-green/30 break-words whitespace-normal h-auto py-2 px-3">
            ...
            <div className="flex space-x-2 min-w-0">
              <Input
                ...
                disabled={loading}
              />
              <Button onClick={handleSendMessage} disabled={!input.trim() || loading} className="bg-crypto-green hover:bg-crypto-green/90 text-primary-foreground flex-shrink-0">
